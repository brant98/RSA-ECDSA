...
big sign_crt_fdh(char* text, big d, big p, big q)//crt模式RSA-FDH
{
	...
	mip->IOBASE = 10;//此处容易出错，转换前不能进行运算的，否则会出错。
	printf("\nfdh Alice is signing the test string\n");
	powmod(m, dp, p, pm[0]);    /* get result mod p */
	powmod(m, dq, q, pm[1]);    /* get result mod q */
	crt(&ch, pm, s);
	return s;
...
void check_fdh(char* text, big s, big e, big n)//FDH进行签名验证
{
	...
	{
		printf("fdh After checking the signature,the result shows that this message is belong to Alice!\n");
	}
	else
	{
		printf("The result shows that this is not signed by Alice!");
